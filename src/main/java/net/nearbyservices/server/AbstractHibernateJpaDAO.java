...
public abstract class AbstractHibernateJpaDAO<K, E> {
	...
	public List<E> findWithTitle(String title) {
		String queryStr = "SELECT h FROM " + entityClass.getName() + " h WHERE h.title = :tit";
		Query query = getEntityManager().createQuery(queryStr, entityClass).setParameter("tit", title);
		List<E> resultList = query.getResultList();
		return resultList;
	}
	...
	public List<String> selectTitles() {
		String queryStr = "SELECT DISTINCT h.title FROM " + entityClass.getName() + " h";
